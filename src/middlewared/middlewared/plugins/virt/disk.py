...
VALID_DISK_FORMATS = ['qcow2', 'qed', 'raw', 'vdi', 'vpc', 'vmdk' ]  # Valid Disk Formats we can export
...
class VirtDeviceService(Service):
    ...
    async def export_disk_image(self, job, data):
        ...
        data_format = data['format'].lower()
        if data_format not in VALID_DISK_FORMATS:
        ...
        targetfile = f"{data['directory']}/vmdisk-{zvolbasename}.{data_format}"
